...
// ---------------------------------------------------------------------------
// Hex returns the hexadecimal encoding of the uuid.
...
// V1 generates a UUID from hardware address, sequence number, and the current time.
func V1() UUID {
	uuid := UUID{}
	now := now()
...
// V3 generates a UUID from the MD5 hash of a namespace UUID and a unique key.
func V3(namespace UUID, key string) UUID {
...
// V5 generates a UUID from the SHA-1 hash of a namespace UUID and a unique key.
func V5(namespace UUID, key string) UUID {
